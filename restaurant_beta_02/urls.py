...
urlpatterns = [
    ...
    url('^testticket2/(?P<pk>.+)/$', views.TestTicket2View.as_view()),  # 根据Ticket_id从DataOrder反向查询单个ticket
    ...
    url('^testAxios/$', views.TestAxiosTicketListView.as_view()),  # 测试后端发送携带cookie的请求
    url('^testAxios2/$', views.TestAxiosTicketListView2),  # 测试后端发送携带cookie的请求(测试简化版)
    url('^testAxios/(?P<pk>.+)/$', views.TestAxiosTicketDetailView.as_view()),  # 测试后端发送携带cookie的请求
    url('^testAxios2/(?P<pk>.+)/$', views.TestAxiosTicketDetailView2),  # 测试后端发送携带cookie的请求(测试简化版)
